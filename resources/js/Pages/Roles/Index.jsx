...
const Index = () => {
  const { roles, filters, title } = usePage().props
    ...
    <div>
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between mb-8">
          <h2 className="text-2xl font-bold">Daftar Jenis Role</h2>
          <div className="flex gap-x-2">
            <ExportButton type="roles" allFields={allFields} />
            <ImportButton type="roles" />
            <Button asChild>
              <Link href="/roles/create"><Plus /> Tambah</Link>
            </Button>
          </div>
        </div>
            ...
            {roles.length > 0 ? (
              roles.map((role, index) => (
                <TableRow key={role.id}>
                  ...
                  {/* <TableCell>{role.status.toString()}</TableCell> */}
                  <TableCell>
                    <Button variant="default" size="sm" className="me-1" asChild>
                      <Link href={`/roles/${role.id}/edit`}><Pencil className="h-4 w-4" /></Link>
                    </Button>
                    <AlertDialog>
